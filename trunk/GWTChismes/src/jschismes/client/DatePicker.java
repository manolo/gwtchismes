...
public class DatePicker extends GWTCDatePickerAbstract implements Exportable {
  ...
  public DatePicker(JavaScriptObject prop) {
    ...
    int cfg = 0;
    if (!jsProp.defined(Const.CONT_ID) && jsProp.getBoolean(Const.DIALOG, true))
      cfg |= CONFIG_DIALOG;
    if (jsProp.getBoolean(Const.RND_BOX, false))
      cfg |= CONFIG_ROUNDED_BOX;
    if (!jsProp.getBoolean(Const.AUTOHIDE, true))
      cfg |= CONFIG_NO_AUTOHIDE;
    if (!jsProp.getBoolean(Const.ANIMATE, true))
      cfg |= CONFIG_NO_ANIMATION;
    if (jsProp.getBoolean(Const.GLASS, true))
      cfg |= CONFIG_BACKGROUND;
    if ("flat".equals(jsProp.get(Const.BUTTONS)))
      cfg |= CONFIG_FLAT_BUTTONS;
    if ("standard".equals(jsProp.get(Const.BUTTONS)))
