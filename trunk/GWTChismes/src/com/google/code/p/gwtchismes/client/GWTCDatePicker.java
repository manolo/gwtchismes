...
public class GWTCDatePicker extends Composite implements ClickListener,
  ...
  private void initialize(int config) {
    ...
    bottonButtons.setCellVerticalAlignment(actualMBtn,
        HasAlignment.ALIGN_MIDDLE);
    bottonButtons.setCellHorizontalAlignment(prevButtons, DockPanel.ALIGN_LEFT);
    bottonButtons
        .setCellHorizontalAlignment(nextButtons, DockPanel.ALIGN_RIGHT);
    bottonButtons.setCellHorizontalAlignment(actualMBtn,
  ...
  public void drawCalendar() {
    ...
    for (int i = weekStart; i < 7; i++) {
      grid.getCellFormatter().setStyleName(0, l,
          GWTCDatePicker.StyleCCellDayNames);
    ...
    for (int i = 1; i < 7; i++) { // each row in the grid
      for (int k = 0; k < 7; k++, j++) { // each day in the week
        int displayNum = (firstWDay < weekStart) ? (j - firstWDay - 6) : (j
            - firstWDay + 1);
  ...
  public void setMinimalDate(Date d) {
    ...
    if (cursorDate.getTime() < minimalDate.getTime()
        || selectedDate.getTime() < minimalDate.getTime())
  ...
  public void setMaximalDate(Date d) {
    ...
    if (cursorDate.getTime() > maximalDate.getTime()
        || selectedDate.getTime() > maximalDate.getTime())
      this.setSelectedDate(d);
  ...
  public void onClick(Widget sender) {
    if (sender == prevMBtn) {
      ...
      setSelectedDate(new Date(cursorDate.getYear(), cursorDate.getMonth(),
          cell.getDay()));
   ...
   */
  static public String formatDate(String format, Date date, String[] months,
      String days[]) {
    /*
     * DateTimeFormat dateFormat = DateTimeFormat.getFormat(format); Window.alert ("" + date + " " + dateFormat.format(date));
     */
...
ng month = months[date.getMonth()];
        String day = days[date.get// String day_4 = day.length() >= 4 ? day.substring(0, 4) : day;
...
"" + leftPadding(String.valueOf(date.getYear() % 100), "0", 2));
  ...
  static private String leftPadding(String text, String character,
      int maxNumberChars) {
